...
Dockerfile is also included in this project. You can build the docker image by running the following command:

```bash
docker build -t crispy-admin-panel .
```

Then you can run the docker image by running the following command:

```bash
docker run -d -p 3000:3000 crispy-admin-panel
```
